...
from app.service.sentences_service import check_type_sentences
...
def get_all_suspicious_content_route(email: str):
    try:
        ...
        return jsonify({'message': suspicious_content})
    except Exception as e:
        print(f'Error: Failed get content to email: {email}')
        return jsonify({'Error': f'Failed get content to email: {email}'})
